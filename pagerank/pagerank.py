...
def transition_model(corpus, page, damping_factor):
    ...
    no_of_linked_pages = len(pagelinks)

    if pagelinks:
        for key in corpus:
            prob_dist_dict[key] = (1-damping_factor)/no_of_pages
        for key in pagelinks:
            prob_dist_dict[key] += damping_factor/no_of_linked_pages
    else:
         for key in corpus:
            prob_dist_dict[key] = 1/no_of_pages
...
def iterate_pagerank(corpus, damping_factor):
    ...
    distribution = {}.fromkeys(corpus.keys(), 1/no_of_pages)
    ...
    while change:
        ...
        for page in corpus:
            distribution[page] = ((1 - damping_factor)/no_of_pages) + (damping_factor * get_sum(corpus, distribution, page))
